...
GIT_ATTRIBUTES = [
    "*." + file_ext + " diff=xltrail" for file_ext in FILE_EXTENSIONS
]
...
class Installer:
    def __init__(self, mode="global", path=None):
        ...
        if mode == "local" and not path:
            raise ValueError(
                "must specify repository path when installing locally"
            )
    ...
    def install(self):
        ...
        self.update_git_file(
            path=self.git_attributes_path, keys=GIT_ATTRIBUTES, operation="SET"
        )
    ...
    def uninstall(self):
        ...
        # when in global mode and gitattributes is empty, update gitconfig and
        ...
        gitignore_keys = self.update_git_file(
            path=self.git_attributes_path, keys=GIT_IGNORE, operation="REMOVE"
        )
        # when in global mode and gitignore is empty, update gitconfig and
        # delete gitignore
    ...
    def get_git_attributes_path(self):
        ...
        core_attributesfile = self.execute(
            ["--get", "core.attributesfile"]
        ).split("\n")[0]
...
class CommandParser:
    ...
    def env(self):
        ...
        p += (
            "LocalWorkingDir="
            + (current_path if is_git_repository(current_path) else "")
            + "\n"
        )
        p += (
            "LocalGitIgnore="
            + (
                os.path.join(current_path, ".gitignore")
        ...
        p += (
            "LocalGitAttributes="
            + (
                os.path.join(current_path, ".gitattributes")
                if is_git_repository(current_path)
                else ""
            )
            + "\n"
